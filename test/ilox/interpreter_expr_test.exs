  ...
  describe "eval_expr/2: equality" do
    property "value == value -> true" do
      check all(value <- literal()) do
        assert {:ok, expression: "true", output: []} =
    ...
    property "value != value -> false" do
      check all(value <- literal()) do
        assert {:ok, expression: "false", output: []} =
                 eval_expr("#{inspect(value)} != #{inspect(value)}")
      end
    end
  ...
  describe "eval_expr/2: binary operations (*/+-)" do
    ...
    property "left_string + right_string -> concatenation" do
      check all(left <- literal(:string), right <- literal(:string)) do
        ...
        assert {:ok, expression: ^expr, output: []} =
                 eval_expr("#{inspect(left)} + (#{inspect(right)})")
      end
    end
  ...
  describe "eval_expr/2: unary operations (!-)" do
    ...
    property "!number -> false" do
      check all(value <- number()) do
        assert {:ok, expression: "false", output: []} = eval_expr("!#{value}")
      end
    end

    property "!string -> false" do
      check all(value <- literal(:string)) do
        assert {:ok, expression: "false", output: []} = eval_expr("!#{inspect(value)}")
      end
    end

    test "!true -> false" do
      assert {:ok, expression: "false", output: []} = eval_expr("!true")
    end

    test "!false -> true" do
      assert {:ok, expression: "true", output: []} = eval_expr("!false")
    end

    test "!nil -> true" do
      assert {:ok, expression: "true", output: []} = eval_expr("!nil")
    end

    property "!!number -> true" do
      check all(value <- number()) do
        assert {:ok, expression: "true", output: []} = eval_expr("!!#{value}")
      end
    end

    property "!!string -> true" do
      check all(value <- literal(:string)) do
        assert {:ok, expression: "true", output: []} = eval_expr("!!#{inspect(value)}")
      end
    end

    test "!!true -> true" do
      assert {:ok, expression: "true", output: []} = eval_expr("!!true")
    end

    test "!!false -> false" do
      assert {:ok, expression: "false", output: []} = eval_expr("!!false")
    end

    test "!!nil -> false" do
      assert {:ok, expression: "false", output: []} = eval_expr("!!nil")
    end
  end

  describe "eval_expr/2: logical operations (and/or)" do
    test "false and 3 -> false" do
      assert {:ok, expression: "false", output: []} = eval_expr("false and 3")
    end

    test "false or 3 -> 3" do
      assert {:ok, expression: "3", output: []} = eval_expr("false or 3")
    end

    test "false or nil or 3 -> 3" do
      assert {:ok, expression: "3", output: []} = eval_expr("false or nil or 3")
    end

    test "false and nil or 3 -> false" do
      assert {:ok, expression: "3", output: []} = eval_expr("false and nil or 3")
    end

    test "true or nil or 3 -> true" do
      assert {:ok, expression: "true", output: []} = eval_expr("true or nil or 3")
    end

    test "true and nil or 3 -> nil" do
      assert {:ok, expression: "3", output: []} = eval_expr("true and nil or 3")
    end

    test "true and !nil and 3 -> 3" do
      assert {:ok, expression: "3", output: []} = eval_expr("true and !nil and 3")
    end
  end

  defp eval_expr(source) do
    case Interpreter.eval_expr(env(), source) do
      {:ok, expr} -> {:ok, expression: expr, output: Process.get(:"$ilox$output", [])}
